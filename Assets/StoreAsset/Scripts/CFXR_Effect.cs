			...
			public void animate(float time)
			{
				...
				if (light != null)
				{
					if (animateIntensity)
					{
						...
						delta = perlinIntensity ? Mathf.PerlinNoise(Time.unscaledTime * perlinIntensitySpeed, 0f) : intensityCurve.Evaluate(delta);
					...
					if (animateRange)
					{
						...
						delta = perlinRange ? Mathf.PerlinNoise(Time.unscaledTime * perlinRangeSpeed, 10f) : rangeCurve.Evaluate(delta);
					...
					if (animateColor)
					{
						...
						delta = perlinColor ? Mathf.PerlinNoise(Time.unscaledTime * perlinColorSpeed, 0f) : colorCurve.Evaluate(delta);
